function scr_update_user_node_connection_speed_limit(x1, y1, x2, y2, new_speed)
{
	...
	for (var i = 0; i < size; i++)
	{
		...
		for (j = 0; j < array_length(connections); j++)
		{
			...
			structure = scr_extract_user_node_connection_key_values(connections[j]);
		...
		connections[j] = scr_create_user_node_connection_key_from_struct(structure);
